				...
				if (pagesxslt == null) {
					...
					if (request.getMethod().equals(HTTPRequestMethod.POST)) {
						if (request.getResourceParameters().containsKey("xslt")
								&& request.getResourceParameters().containsKey("xsd")) {// AQUÍ COMPRUEBO SI LA REQUEST
																						// TIENE XSLT Y XSD
							if (request.getResourceParameters().get("xsd") != null) {// AQUI COMPRUEBO SI ME HAN PASADO
																						// UN XSD QUE NO ES NULL
								...
								if (xsldexist.exist(xsdl)) {// AQUÍ COMPRUEBO SI EXISTE EL XSD EN LA BD
									...
									String[] contenido = content.split("=");// EN CONTENIDO TENEMOS TAMBIEN EL XSD Y
																			// SOLO DEBERÍAMOS TENER EL CONTENIDO DEL
																			// XSLT TENEMOS ALGO DEL ESTILO
																			// XSD=JCDBHUVCW XSLT=????????????
									content = contenido[2];// AQUI COGEMOS SOLO EL CONTENIDO DEL XSLT QUE ESTA EN LA
...
=======
					if (request.getMethod().equals(HTTPRequestMethod.DELETE)) {
						...
						if (uuid == null) {
							response.setStatus(HTTPResponseStatus.S400);
>>>>>>> ec1144031c4209a2c4ffad48c54b462b3ba22351
						} else {
							if (contenido == null) {
								pagesxslt.deletePage(uuid);
								response.setStatus(HTTPResponseStatus.S200);
	...
	private String listarPaginas(HtmlController pages) throws Exception {
		...
		if (!paginas.isEmpty()) {
			...
			while (it.hasNext()) {
				...
				identificador += "<li><a href='html?uuid=" + itpagina + "'>" + itpagina + "</a></li>";
			}
			identificador += "</ul>";
		} else {
